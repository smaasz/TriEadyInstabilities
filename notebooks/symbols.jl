...
floataside(@bind selectedschemes selectschemes(); top=100)
...
function plotinstabilities(df, Ri, grid_t, hmt_scheme; Nz, θU, β, a, Vᵘ, Vᵇ, hst_scheme, scatterpts=nothing)
	...
	for row in eachrow(subdf)
		...
		if !isnothing(scatterpts)
		scatter!(scatterpts[1] ./ fₛ, scatterpts[2] .* (sqrt(N²) / abs(M²) ); markercolor=color, markersize=6, label=nothing)
	end
	end
	...
	plot!(legend_column=-1, legend=:outerbottom, size=size, leftmargin=Plots.AbsoluteLength(10.0), bottommargin=Plots.AbsoluteLength(-10.0))
	p
end
...
function plotinstabilities(df, Ri; Nz, θU, β, a, Vᵘ, Vᵇ, hst_scheme)
	...
	plot!(legend_column=4, legend=:outerbottom, size=size, leftmargin=Plots.AbsoluteLength(10.0), bottommargin=Plots.AbsoluteLength(-10.0))
	p
end
